<!-- docs/_sidebar.md -->

- [Home](/)

- **Arquitetura & Reutilização de Software**
  - [4. Desenho de Software (Arquitetura & Reutilização de Software)](/)
    - [4.1. Reutilização de Software](/ArquiteturaReutilizacao/4.1.ReutilizacaoDeSoftware.md)
      - [4.1.1. Componentes de UI](/ArquiteturaReutilizacao/4.1.1.Componentes.md)
      - [4.1.2. Componentes de Layout](/ArquiteturaReutilizacao/4.1.2.Layout.md)
      - [4.1.4. Pacotes de uso compartilhado entre camadas com o Turborepo](/ArquiteturaReutilizacao/4.1.4.Turborepo.md)
    - [4.2. Participações - Arquitetura & Reutilização de Software](/ArquiteturaReutilizacao/4.2.ParticipacoesArqReutilizacao.md)
